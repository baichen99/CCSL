...
func (s *SignService) GetSignList(parameters utils.GetSignListParameters) (signs []models.Sign, count int,  err error) {
    ...
    //query := db.Table("signs").Select("videos.construct_type, videos.construct_words,videos.left_sign, videos.right_sign, videos.video_path, sign.id, sign.name").Joins("LEFT JOIN video ON ")
